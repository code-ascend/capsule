...
`--commit` пересобирает squashfs-образ внутри бинарника, включая все изменения из overlay. `--update` запускает скрипт обновления, описанный в секции `update:` конфига, и автоматически коммитит результат. Оба требуют root.
